// src/main/handlers/inventory.js — v2.2 stable + multi-tenant safe + local stock apply
...
function buildJsonHeaders(extra = {}) {
  ...
  if (tenantId && /^[0-9a-f-]{36}$/i.test(String(tenantId))) headers['x-tenant-id'] = String(tenantId);
  return headers;
}
...
async function apiInventorySummary(sessionId) {
  ...
  return {
    ...
    lines: lines.map(l => ({
      ...
      barcode: normCode(l.barcode ?? l.code_barre ?? l.code_barres ?? l.ean ?? l.code ?? ''),
      counted_total: Number(l.counted_total ?? l.count ?? l.qty ?? 0),
      price: Number(l.prix ?? l.price ?? 0),
    })),
  };
}
async function apiInventoryFinalize(apiBase, _tokenIgnored, sessionId, body = {}) {
  const sid = String(sessionId || '').trim();
  ...
  const r = await fetch(`${apiBase}/inventory/${encodeURIComponent(sid)}/finalize`, {
    method: 'POST',
    headers,
...
function applySummaryToLocal(lines) {
  ...
  let produits = [];
  try { produits = db.prepare(produitsQuery).all(); }
  catch (e) {
  ...
  const coverage = produits.length ? (mapped.size / produits.length) : 0;
  ...
  if (coverage >= 0.7) {
    ...
    tx();
    console.log(`[inventory] stocks locaux MAJ (full reset) — matched ${mapped.size}/${produits.length} (${(coverage*100)|0}%)`);
  } else {
    const tx = db.transaction(() => {
      for (const [id, s] of mapped.entries()) stmtSetStock.run(s, id);
    });
    tx();
    console.warn(`[inventory] couverture faible → MAJ partielle sans reset (matched ${mapped.size}/${produits.length})`);
  }
...
module.exports = function registerInventoryHandlers(ipcMain) {
  ...
  const doCountAdd = async (_e, payload = {}) => {
    ...
    const { uuid: remoteUUID, barcode } = resolveRemoteProductUUID(localNum);

    if (!remoteUUID) {
      try {
        const sync = require('../sync');
        if (sync && typeof sync.pullRefs === 'function') {
          await sync.pullRefs();
          const again = resolveRemoteProductUUID(localNum);
          if (again.uuid) {
            return apiInventoryCountAdd({ sessionId, product_uuid: again.uuid, qty, user, device_id });
          }
        }
      } catch (e) {
        console.warn('[inventory] tentative pullRefs échouée:', e?.message || e);
      }

      const hint = barcode ? ` (barcode: ${barcode})` : '';
      throw new Error(
        `inventory:count-add MAPPING_MISSING — aucun remote_uuid pour produit local ${localNum}${hint}. ` +
        `Va dans Paramètres → "Pull TOUT (Neon→local)" ou lance l’app sans SKIP_BOOTSTRAP/SKIP_HYDRATE, puis réessaie.`
      );
    }
  ...
  ipcMain.handle('inventory:finalize', async (_evt, { sessionId, user } = {}) => {
    ...
    try {
      const out = await apiInventoryFinalize(API, null, String(sessionId), { user });

      // Update local stock from summary
      try {
        const sum = await apiInventorySummary(String(sessionId));
        applySummaryToLocal(sum.lines);
      } catch (e) {
        console.warn('[inventory] applySummaryToLocal failed:', e?.message || e);
      }
