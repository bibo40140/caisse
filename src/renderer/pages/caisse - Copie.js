...
const taxo = await window.electronAPI.getCategoriesProduits().catch(()=>null);

// --- Compatibilité avec l'ancienne version ---
// clés "fixes" (l'ancienne version lisait directement ces champs)
const FAM_NAME_KEY = 'famille_effective_nom';
const CAT_NAME_KEY = 'categorie_effective_nom';
// pas d'IDs fiables dans l'ancienne version
const FAM_ID_KEY = null;
const CAT_ID_KEY = null;

// maps vides (évite ReferenceError, et servent si on les remplit plus tard)
const famIdToName = new Map();
const catIdToName = new Map();
const catIdToFamName = new Map();

// ➕ Nouvelle map : nom de catégorie -> nom de famille (dérivé de la taxo)
const catNameToFamName = new Map();

// utilitaire pour parcourir des structures possiblement imbriquées
const asArray = (x) => Array.isArray(x) ? x : (x ? [x] : []);

// Remplissage catName → famName à partir de la taxo (plate OU imbriquée)
try {
  const fams = taxo?.familles || taxo?.families || taxo?.data?.familles || [];
  const cats = taxo?.categories || taxo?.data?.categories || [];

  // Catégories "plates" contenant un nom de famille
  for (const c of asArray(cats)) {
    const cName = (c?.nom ?? c?.name ?? c?.label ?? '').trim();
    const fName = (c?.famille_nom ?? c?.family_name ?? c?.rayon_nom ?? c?.group_name ?? '').trim();
    if (cName && fName) catNameToFamName.set(cName, fName);
  }

  // Familles avec catégories imbriquées
  for (const f of asArray(fams)) {
    const fName = (f?.nom ?? f?.name ?? f?.label ?? '').trim();
    if (!fName) continue;
    const nested =
      f.categories || f.cats || f.children || f.sous_categories ||
      f.sub_categories || f.subCategories || [];
    for (const c of asArray(nested)) {
      const cName = (c?.nom ?? c?.name ?? c?.label ?? '').trim();
      if (cName) catNameToFamName.set(cName, fName);
    }
  }
} catch {}

console.log('[CAISSE] diag compat:', { catNameToFamName: catNameToFamName.size });
// --- compat : fonction pour récupérer le nom de catégorie d’un produit ---
const getCat = (p) => {
  // 1) ancienne voie directe (champ texte sur le produit)
  if (CAT_NAME_KEY && typeof p[CAT_NAME_KEY] === 'string' && p[CAT_NAME_KEY].trim()) {
    return p[CAT_NAME_KEY].trim();
  }
  // 2) via id -> nom si jamais on en a un jour
  if (CAT_ID_KEY && Number.isFinite(Number(p[CAT_ID_KEY]))) {
    const id = Number(p[CAT_ID_KEY]);
    const n = catIdToName.get(id);
    if (n) return n;
  }
  return null;
};
...
// Accès lisibles
const getFam = (p) => {
  // 1) ancienne voie directe (si jamais présente)
  if (FAM_NAME_KEY && typeof p[FAM_NAME_KEY] === 'string' && p[FAM_NAME_KEY].trim())
    return p[FAM_NAME_KEY].trim();

  // 2) via id famille (pas utilisé ici mais on garde le code compatible)
  if (FAM_ID_KEY && Number.isFinite(Number(p[FAM_ID_KEY]))) {
    const id = Number(p[FAM_ID_KEY]);
    const n = famIdToName.get(id);
    if (n) return n;
  }

  // 3) via id catégorie (non utilisé dans l’ancienne version)
  if (CAT_ID_KEY && Number.isFinite(Number(p[CAT_ID_KEY]))) {
    const cid = Number(p[CAT_ID_KEY]);
    const n = catIdToFamName.get(cid);
    if (n) return n;
  }

  // 4) ✅ NOUVEAU : via NOM de catégorie → catName -> famName
  if (CAT_NAME_KEY && typeof p[CAT_NAME_KEY] === 'string' && p[CAT_NAME_KEY].trim()) {
    const cname = p[CAT_NAME_KEY].trim();
    const n = catNameToFamName.get(cname);
    if (n) return n;
  }
...
console.log('[CAISSE] Keys détectées :', { FAM_NAME_KEY, CAT_NAME_KEY, FAM_ID_KEY, CAT_ID_KEY });
console.log('[CAISSE] Maps construites :', { famIdToName: famIdToName.size, catIdToName: catIdToName.size });
console.log('[CAISSE] échantillon produits :', (produits||[]).slice(0,3).map(p=>({
  famNom: FAM_NAME_KEY ? p[FAM_NAME_KEY] : undefined,
  famId:  FAM_ID_KEY   ? p[FAM_ID_KEY]   : undefined,
  catNom: CAT_NAME_KEY ? p[CAT_NAME_KEY] : undefined,
  catId:  CAT_ID_KEY   ? p[CAT_ID_KEY]   : undefined,
  nom: p.nom
})));

// Petit badge de diag si rien n'est détecté (temporaire)
setTimeout(() => {
  const card = document.querySelector('.filters-card');
  if (!card) return;
  const famCount = new Set((produits||[]).map(getFam).filter(Boolean)).size;
  const catCount = new Set((produits||[]).map(getCat).filter(Boolean)).size;
  const note = document.createElement('div');
  note.style.cssText = 'font-size:12px;color:#666;margin-top:6px';
  note.innerHTML = `Diag familles/cats → familles: <b>${famCount}</b>, catégories: <b>${catCount}</b>`;
...
const matchFamCat = (p) => {
  const famEff = getFam(p);
  const catEff = getCat(p);
...
function renderFamillesBar() {
  ...
  // familles depuis les produits…
  let familles = Array.from(new Set((produits || []).map(getFam).filter(Boolean)));

if (familles.length === 0 && catNameToFamName.size) {
  familles = Array.from(new Set(catNameToFamName.values()));
}

if (familles.length === 0) {
  const fams = taxo?.familles || taxo?.families || taxo?.data?.familles || [];
  familles = asArray(fams)
    .map(f => (f?.nom ?? f?.name ?? f?.label ?? '').trim())
    .filter(Boolean);
}


  familles.sort((a,b) => a.localeCompare(b, 'fr'));
  ...
  wrap.querySelectorAll('.btn-fam').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (f === '') {
        ...
        familleActive = (familleActive === f) ? null : f;
...
console.log('[CAISSE] diag familles:', {
  famKeys: { FAM_NAME_KEY, FAM_ID_KEY },
  catKeys: { CAT_NAME_KEY, CAT_ID_KEY },
  famIdToName: famIdToName.size,
  catIdToName: catIdToName.size,
  catIdToFamName: catIdToFamName.size
});
...
function renderCategoriesBar() {
  ...
  .filter(p => getFam(p) === familleActive)
  .map(getCat)
  .filter(Boolean)))
...
cats = Array.from(new Set((produits || [])
  ...
  wrap.querySelectorAll('.btn-cat-square').forEach(btn => {
    ...
    window.viderPanier = ({ skipConfirm = false } = {}) => {
      ...
      afficherPanier();
      document.getElementById('search-produit')?.focus();
      window.__catalogProduits = produits;
