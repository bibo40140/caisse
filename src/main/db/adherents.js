...
function ajouterAdherent(data) {
  const stmt = db.prepare(`
    INSERT INTO adherents 
      (nom, prenom, email1, email2, telephone1, telephone2, adresse, code_postal, ville, foyer, tranche_age, archive)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
  `);
  stmt.run(
    data.nom, data.prenom, data.email1, data.email2, data.telephone1,
    data.telephone2, data.adresse, data.code_postal, data.ville,
    data.foyer, data.tranche_age
  );
}
...
function modifierAdherent(data) {
  const stmt = db.prepare(`
    UPDATE adherents SET 
      ...
      adresse = ?, code_postal = ?, ville = ?, foyer = ?, tranche_age = ?
    WHERE id = ?
  `);
  stmt.run(
    data.nom, data.prenom, data.email1, data.email2, data.telephone1,
    data.telephone2, data.adresse, data.code_postal, data.ville,
    data.foyer, data.tranche_age, data.id
  );
}
