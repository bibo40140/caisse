...
// NOTE: on n'importe PAS inventoryRoutes ici pour éviter les conflits,
// car on expose ci-dessous des endpoints inventaire inline dans ce fichier.

// Middleware d’auth
import { authRequired } from './middleware/auth.js';

// --- UUID validator global ---
const UUID_RE =
...
/* ============================================
 * Helper stock courant (multi-tenant)
 * ==========================================*/
 ...
 * inventory_snapshot, inventory_adjust
 * =======================================================*/
...
app.post('/inventory/start', authRequired, async (req, res) => {
  ...
  try {
    ...
    // Snapshot des stocks au démarrage
...
app.post('/inventory/:id/finalize', authRequired, async (req, res) => {
  ...
  try {
    ...
    for (const r of agg.rows) {
      ...
      // 4.b Mouvement de stock si delta ≠ 0 (idempotent)
      if (delta !== 0) {
...
app.post('/sync/push_ops', authRequired, async (req, res) => {
  ...
  console.log('[API] /sync/push_ops received:', {
    deviceId,
    count: ops.length,
    tenantId,
  });
      ...
      try {
        ...
        if (r.rowCount > 0) {
          ...
          if (typeof fromDb === 'string') {
            try { fromDb = JSON.parse(fromDb); } catch { /* noop */ }
          }
      ...
      switch (op.op_type) {
        ...
        case 'sale.line_added': {
          ...
          if (chk.rowCount === 0) {
            ...
            console.log(
              '    [+] ligne_vente ajoutée',
              { vente: venteId, produit: produitId, qte: quantite }
            );
          } else {
        ...
        case 'inventory.adjust': {
          ...
          if (!produitId) {
            console.warn(
              '    [!] inventory.adjust ignorée — produitId non-UUID:',
              p.produitId
            );
            break;
          }
          if (!Number.isFinite(delta) || delta === 0) {
            console.warn(
              '    [!] inventory.adjust ignorée — delta invalide:',
              p.delta
            );
            break;
          }
...
app.post('/sync/bootstrap', authRequired, async (req, res) => {
  try {
    ...
    if (headerTenant && !UUID_RE.test(headerTenant)) {
      return res
        .status(400)
        .json({ ok: false, error: 'invalid x-tenant-id (must be UUID)' });
    }
    const tenantId = req.tenantId || headerTenant;
    if (!tenantId) {
      return res.status(400).json({ ok: false, error: 'tenant_missing' });
    }
    ...
    try {
      ...
      await client.query('ROLLBACK');
      console.error('POST /sync/bootstrap error:', err);
      return res
        .status(500)
    ...
    console.error('POST /sync/bootstrap wrapper error:', e);
    return res.status(500).json({ ok: false, error: e.message });
  }
});
